...
namespace AoC2025_Day3
{
    internal class AocProgram : AoCDay, ISolvable
    {
        public void SolvePart1()
        {
            ...
            var result = rawInput.Select(input => long.Parse(GetXDigitLongJoltage(input, 2)))
                .PrintEach()
                .Sum();
        ...
        {
            var sum = Helper.Helper.getInputAsLinesOfCurrentDay(day)
        ...
        private static string GetXDigitLongJoltage(string input, int expectedLength)
        {
            ...
            var searchLength = input.Length - expectedLength + 1;

            var largestDigit = input.Take(searchLength).Max();
            var largestIndex = input.IndexOf(largestDigit);

            var remainingInput = input.Substring(largestIndex + 1);
            return largestDigit + GetXDigitLongJoltage(remainingInput, expectedLength - 1);
        }
    }
}
...
// Functions below are older versions, kept for reference
