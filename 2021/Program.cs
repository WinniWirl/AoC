...
class AdventOfCode{
    ...
    private static void runDay(string day){
        Type? classOfDay = Type.GetType($"Day{day}.AOCProgram");
        if (classOfDay == null) {
        ...
        string? result = null;
        if ( userInputSolve == "1") { result = program.solvePart1(); }
        else if ( userInputSolve == "2") { result = program.solvePart2(); }
        // if (result != null){
    ...
    private static async Task checkForInputOfDay(string day)
    {
        ...
        try
        {
            sr = new StreamReader($".\\2021\\inputs\\day{day}.txt");
            line = sr.ReadLine();
            if (line == null)
            {
    ...
    private static async Task downloadInputOfDay(string day){
        ...
        try{
            sw = new StreamWriter($".\\2021\\inputs\\day{day}.txt");
            ...
            Console.WriteLine($"Downloaded input of day {day} successfully!");
        }
        catch(Exception e)
        {
            Console.WriteLine("Exception: " + e.Message);
